...
app.get('/charities', function(request, response, next) {
    ...
    const extraParameters =
        '&pageSize=30&rated=true&fundraisingOrgs=true&state=NY&minRating=4&maxRating=4&scopeOfWork=REGIONAL&sort=NAME%3AASC'
    ...
    axios
        ...
        .then(charities => {
            ...
            let charityNames = charities.data.map(
...
app.post('/orderData', (request, response) => {
    let charityRef = db
        .collection('donationsToCharities')
        .doc(request.body.charity)
...
function bulkUpdateMonthlyDonations(monthArr, uid) {
    ...
    monthArr.forEach((month, index) => {
        transactionCol
            .where('date', '>', monthArr[index])
            .where('date', '<', `${monthArr[index]}-32`)
            .get()
            .then(snapshot => {
                let donation = 0
                snapshot.forEach(doc => {
...
function singleUpdateMonthlyDonations(month, uid, donation) {
    ...
    monthDoc.get().then(snapshot => {
        let monthlyDonation = snapshot.data()
            ? snapshot.data().totalDonations + +donation
            : +donation
...
function updateTotalDonations(donation, uid) {
    ...
    db.runTransaction(t => {
        return t.get(userDoc).then(doc => {
            let newDonationAmount = doc.data().totalDonations
                ? +doc.data().totalDonations + +donation
                : +donation
    ...
    db
        ...
        .then(doc => {
            ...
            keys.forEach(key => {
                db.runTransaction(t => {
                    return t
                        .get(db.collection('charities').doc(key))
                        .then(charityDoc => {
                            let existingDonationAmount = charityDoc.data()
                                .totalDonations
                                ? +charityDoc.data().totalDonations
                                : 0
                            let newDonationAmount =
                                existingDonationAmount +
                                donation * doc.data()[key]
                            newDonationAmount = +newDonationAmount.toFixed(4)
                            t.update(db.collection('charities').doc(key), {
                                totalDonations: newDonationAmount,
                            })
                            calculateDonationsToCharities(
                                key,
                                uid,
                                donation * doc.data()[key],
                            )
                            storeUserDonationsToCharities(
                                key,
                                uid,
                                donation * doc.data()[key],
                            )
...
function calculateDonationsToCharities(charity, user, donation) {
    let donationsToCharityByUser = db
        .collection('donationsToCharities')
        .doc(charity)
        .collection('donationsToCharity')
        .doc(user)

    donationsToCharityByUser.get().then(charityDoc => {
        if (!charityDoc.data()) {
            donationsToCharityByUser.set({
                totalDonations: donation,
                uid: user,
            })
        } else {
            db.runTransaction(t => {
                return t.get(donationsToCharityByUser).then(doc => {
                    let newDonationAmount = doc.data()
                        ? +doc.data().totalDonations + donation
                        : donation
                    newDonationAmount = +newDonationAmount.toFixed(2)
                    t.update(donationsToCharityByUser, {
                        totalDonations: newDonationAmount,
                    })
                })
            })
...
function storeUserDonationsToCharities(charity, user, donation) {
    let donationByUserDoc = db.collection('donationsFromUsers').doc(user)

    donationByUserDoc.get().then(userDoc => {
        if (!userDoc.data()) {
            donationByUserDoc.set({ [charity]: donation })
        } else {
            db.runTransaction(t => {
                return t.get(donationByUserDoc).then(doc => {
                    let newDonationAmount = doc.data()[charity]
                        ? +doc.data()[charity] + donation
                        : +donation
                    newDonationAmount = +newDonationAmount.toFixed(2)
                    t.update(donationByUserDoc, {
                        [charity]: newDonationAmount,
                    })
                })
            })
        }
